...
async def _main() -> None:
    parser = argparse.ArgumentParser(description="Show why current scan candidates are rejected.")
    ...
    for row in report["near_misses"]:
        print(
            ...
            f"vol={row['volume_ratio']:.1f}x value={row['trading_value_krw'] / 100_000_000:,.0f}억"
        )
        print(f"  탈락: {', '.join(row['risk_labels'])}")
